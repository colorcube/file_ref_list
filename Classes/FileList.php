<?php
namespace Colorcube\FileRefList;
...
use Colorcube\FileRefList\Controller\FileReferencesListController;
use Colorcube\FileRefList\Resource\FileRepository;
...
use Colorcube\FileRefList\Controller\FileListController;
use TYPO3\CMS\Extbase\Utility\DebuggerUtility;

/**
 * Class for rendering of File>FileRefList
 */
class FileList extends AbstractRecordList
{
    ...
    /**
     * @var FileReferencesListController
     */
     ...
     * Construct
     *
     * @param FileReferencesListController $fileListController
     */
    public function __construct(FileReferencesListController $fileListController)
    {
        parent::__construct();
     ...
     */
    public function start($pid, $pointer, $sort, $sortRev, $clipBoard = false, $bigControlPanel = false)
    {
        $this->id = $pid;
        $this->counter = 0;
        $this->totalbytes = 0;
        $this->JScode = '';
        $this->HTMLcode = '';
     //Remove   $this->path = $folderObject->getReadablePath();
        $this->sort = $sort;
    ...
    {
        $this->HTMLcode .= $this->getTable('path,fileext,tstamp,size,rw,_REF_');
    }
    ...
    public function getTable($rowlist)
    {
        ...
        if ($this->id) {
            ...
            $files = $fileRepository->findByPid($this->id);
            ...
            foreach ($this->fieldArray as $v) {
                if ($v === '_CLIPBOARD_' && $this->clipBoard) {
                    ...
                    if (!empty($elFromTable)) { //Remove? && $this->folderObject->checkActionPermission('write')) {
                        $addPasteButton = true;
                        $elToConfirm = [];
...
//                            }
                    ...
                    // Normal row:
                    $theT = $this->linkWrapSort(htmlspecialchars($this->getLanguageService()->getLL('c_' . $v)), $this->id, $v);
                    $theData[$v] = $theT;
                }
            }
		...
		-->
			<div class="table-fit">
				<table class="table table-striped table-hover" id="typo3-file_ref_list">
					' . $out . '
				</table>
			</div>';
        } else {
    ...
    public function formatFileList(array $files)
    {
        ...
        foreach ($files as $fileReference) {
            ...
            # DebuggerUtility::var_dump($fileObject);
            ...
            $path = trim($fileObject->getIdentifier());
            // The icon with link
    ...
    /**
     * Wraps the directory-titles ($code) in a link to file_ref_list/Modules/FileRefList/index.php (id=$path) and sorting commands...
     *
    ...
    public function linkWrapSort($code, $folderIdentifier, $col)
    {
        ...
        $href = BackendUtility::getModuleUrl('file_FileRefListList', $params);
    ...
    public function makeEdit($fileOrFolderObject)
    {
        ...
        if ($fileOrFolderObject->checkActionPermission('delete')) {
            ...
            $cells['delete'] = '<a href="#" class="btn btn-default t3js-file_ref_list-delete" data-content="' . htmlspecialchars($confirmationMessage)
